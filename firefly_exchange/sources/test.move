...
module firefly_exchange::test {
    ...
    public native fun secp256k1_verify(signature: &vector<u8>, public_key: &vector<u8>, msg: &vector<u8>): bool;
    ...
    public entry fun verifySignature(signature: vector<u8>, public_key: vector<u8>, hashed_msg: vector<u8>) {
        ...
        if(length == 64){ // ed25519 has signature length 654
            is_verified = ed25519_verify(&signature, &public_key, &hashed_msg);
        }else { // secp256k1 has signature length 65
            is_verified = secp256k1_verify(&signature, &public_key, &hashed_msg);
        };
